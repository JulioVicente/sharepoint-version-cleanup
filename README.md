...
O lançador de `main` instala a versão estável **v1.4.9**, com componentes fixados nessa tag e verificação SHA256. A [release v1.4.9](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/tag/v1.4.9) reconhece também o timeout encapsulado pelo PnP: quando o retry recupera a chamada, o resultado é sucesso e a recuperação fica auditada. Se restarem falhas por arquivo ou biblioteca, retorna resultado parcial e permite concluir a instalação com agendamento em simulação. A produção exige validação e aprovação posteriores. Mantém a retomada de simulações, unidades automáticas de tamanho e cinco retries com esperas-base de 2, 4, 8, 16 e 32 segundos. Em JSON existente, use `Retry.MaxRetries: 5`. Consulte [recuperação e diagnóstico](TROUBLESHOOTING.md). Para outra revisão, use `-ReleaseVersion` ou `-RepositoryRawUrl` na execução por arquivo. Para um checkout local, regenere o manifesto com `& .\tools\Update-ReleaseManifest.ps1` e execute `& .\bootstrap.ps1`.

O arquivo [sharepoint-version-cleanup-v1.4.9.ps1](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/download/v1.4.9/sharepoint-version-cleanup-v1.4.9.ps1) é o mesmo lançador, com a versão no nome. Depois de baixá-lo, use `-Uninstall` para desinstalar ou `-CleanInstall` para limpar e abrir um novo wizard. Veja [comandos e dados preservados](INSTALL_DETAILS.md#desinstalação-e-instalação-limpa).
