...
O lançador de `main` instala a versão estável **v1.4.3**, com componentes fixados nessa tag e verificação SHA256, sem parâmetros adicionais. A [release v1.4.3](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/tag/v1.4.3) separa Graph e PnP em processos para evitar conflitos de DLLs e valida/repara dependências antes do login. Consulte também as orientações sobre [consentimento e dependências](TROUBLESHOOTING.md). Para selecionar outra revisão, use `-ReleaseVersion` ou `-RepositoryRawUrl` na execução por arquivo. Para testar um checkout local, regenere o manifesto com `& .\tools\Update-ReleaseManifest.ps1` e execute `& .\bootstrap.ps1`.
