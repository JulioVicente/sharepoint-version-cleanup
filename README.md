...
O lançador de `main` instala a versão estável **v1.4.8**, com componentes fixados nessa tag e verificação SHA256, sem parâmetros adicionais. A [release v1.4.8](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/tag/v1.4.8) retoma simulações interrompidas, reaproveitando análises válidas e exibindo quantas foram recuperadas, sem duplicar totais. Reavalia arquivos novos, alterados, com falha ou análise vencida. Mantém o retry padrão de cinco repetições, com esperas-base de 2, 4, 8, 16 e 32 segundos. Em configurações existentes, altere `Retry.MaxRetries` para `5` para adotar o novo padrão. Mantém a pequena variação aleatória e respeita `Retry-After`. Também exibe automaticamente o espaço estimado e liberado em B, KB, MB, GB, TB ou unidades maiores no wizard, CLI, piloto e email, preservando os bytes do JSON. Também trata o limite de exclusões como uma pausa planejada: mostra o lote parcial, preserva a retomada e permite aprovar o agendamento sem interromper o wizard. Falhas reais continuam bloqueando a aprovação. Mantém `-Uninstall`, `-CleanInstall`, o isolamento Graph/PnP e o reparo de dependências. Consulte [limites e recuperação](TROUBLESHOOTING.md). Para selecionar outra revisão, use `-ReleaseVersion` ou `-RepositoryRawUrl` na execução por arquivo. Para testar um checkout local, regenere o manifesto com `& .\tools\Update-ReleaseManifest.ps1` e execute `& .\bootstrap.ps1`.

O arquivo [sharepoint-version-cleanup-v1.4.8.ps1](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/download/v1.4.8/sharepoint-version-cleanup-v1.4.8.ps1) é o mesmo lançador, com a versão no nome. Depois de baixá-lo, use `-Uninstall` para desinstalar ou `-CleanInstall` para limpar e abrir um novo wizard. Veja [comandos e dados preservados](INSTALL_DETAILS.md#desinstalação-e-instalação-limpa).
