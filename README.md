...
O lançador de `main` instala a versão estável **v1.4.4**, com componentes fixados nessa tag e verificação SHA256, sem parâmetros adicionais. A [release v1.4.4](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/tag/v1.4.4) acrescenta `-Uninstall` e `-CleanInstall`, com backup da configuração e do estado e preservação dos logs. Mantém o isolamento de Graph/PnP e o reparo de dependências. Consulte as orientações sobre [consentimento e dependências](TROUBLESHOOTING.md). Para selecionar outra revisão, use `-ReleaseVersion` ou `-RepositoryRawUrl` na execução por arquivo. Para testar um checkout local, regenere o manifesto com `& .\tools\Update-ReleaseManifest.ps1` e execute `& .\bootstrap.ps1`.

O arquivo [sharepoint-version-cleanup-v1.4.4.ps1](https://github.com/JulioVicente/sharepoint-version-cleanup/releases/download/v1.4.4/sharepoint-version-cleanup-v1.4.4.ps1) é o mesmo lançador, com a versão no nome. Depois de baixá-lo, use `-Uninstall` para desinstalar ou `-CleanInstall` para limpar e abrir um novo wizard. Veja [comandos e dados preservados](INSTALL_DETAILS.md#desinstalação-e-instalação-limpa).
