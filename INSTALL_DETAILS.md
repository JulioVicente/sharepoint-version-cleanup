...
O bootstrap instala componentes da tag `v1.3.7` por padrão. Para fixar também o lançador, troque `main` por `v1.3.7` na URL do comando. O parâmetro `-ReleaseVersion` seleciona outra tag no bootstrap; `-RepositoryRawUrl` permite usar um commit/origem com manifesto compatível.
