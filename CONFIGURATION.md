...
O email identifica o lote como **PAUSADO PELO LIMITE**. A auditoria registra `RunCompleted` com `Outcome=Deferred`, e o resumo diário o contabiliza em `RunsDeferred`, separado de `RunsFailed`. Uma falha por item ocorrida antes de atingir o limite resulta em `Status=Partial` e não habilita aprovação do piloto como lote sem falhas.

Com `-PassThru`, `Partial` retorna o relatório sem lançar exceção; a CLI sem `-PassThru` retorna código 2 depois de gravar os resultados, permitindo as novas tentativas do Agendador. O wizard oferece repetir os pendentes imediatamente ou concluir a instalação em simulação. Responder N nessa pergunta não cancela a instalação. Não permite aprovar exclusões nem produção a partir de uma simulação parcial. O email exibe **PARCIAL - COM PENDENCIAS** e a auditoria diária conta `RunsPartial`. Autenticação inicial, configuração, lock e impossibilidade de salvar estado/relatório continuam interrompendo o fluxo.

Timeout recuperado pelo retry não entra em `FilesFailed`, `Errors` ou `Error`: o arquivo segue normalmente e a execução pode terminar com `Status=Completed` e email **SUCESSO**. A auditoria preserva `RequestRetry` e registra `RequestRecovered` com o número de tentativas; o resumo diário informa `RequestsRecovered`. O transcript pode conter as mensagens das tentativas anteriores, mesmo com resultado final bem-sucedido. A mensagem específica de `HttpClient.Timeout` também é reconhecida quando o PnP a encapsula num erro PowerShell sem o tipo original. A espera entre retries não altera o timeout de 100 segundos reportado pelo cliente HTTP.
